...
* `sensor_msgs/PointCloud2` on `/lidar_points` (`frame_id = hesai_lidar`) in five of the six
  ...
  with empty `(0,0,0)` slots. The organizers confirmed (23.09) that the control data may use
  either (topic, frame) pair and that all recordings come from the same LiDAR. The node
  subscribes to both names and auto-discovers any other PointCloud2 topic; it processes one
  input at a time, switches when the active one falls silent, and restarts the detector (scene
  state and mount calibration) for every new recording — a new topic, frame id or a jump of the
  header stamps (README "How a bag is processed"). The frame id is bridged to `resense_lidar`
...
the recorded mount (EXPERIMENTS.md §6). Cost: one extra rail/bed measurement every 10th frame during
...
**Experimental far-rail cross-check (Sprint 1; disabled by default).** With
...
track is not usable. The opt-in central near-bed experiment below is disabled in the shipped
configuration until its false-positive cost is measured on the real ride.
...
**Central near-bed objects (Sprint 2, opt-in; not production default).** Set
`lowobj.near_enabled: true` explicitly to test a separate low path accepting anomalies entirely in
...
The excess over the locally offset bed template must exceed `near_min_excess = 0.05 m`;
clustering uses the existing tight low radius and needs ≥ 5 voxels, an observed width of
0.24–0.55 m, an observed along-track extent of at least 0.18 m and ≤ 0.75 m along it. Each
2 m bin must also have bed support spanning at least 20 lateral template bins, so an object
cannot create its own local reference across a missing-bed gap. The vertical extent is not
required because a valid top-surface return can have zero observed height. The additional
footprint and full-bed-support gates reject the flat/sliver bed structures
that dominated the first real-ride opt-in run while retaining a volumetric 30×30×10 cm object
model. These clusters can remain below the rail head. The existing
...
linear hardware and small fluctuations. The existing 5-hit low-object confirmation and
foot-of-corridor suppression apply. A compact inductor or short raised cover with the
same 3-D returns as a foreign object cannot be distinguished by this geometry alone;
...
real frame, so `ego_speed_estimate` is null). Without it and with the shipped
`accumulation.estimate_speed: false`, the source is `"none"` and **nothing is merged**.
Opting in to the LiDAR estimator may instead yield `"estimated"` when confident, or `"none"`
when it is uncertain: a wrong shift smears, and the single-frame pipeline is the safe fallback. Below
...
| `track.rails_far_check_enabled` | false | experimental far-rail cross-check for station-wall curvature (§3.1); opt-in, real-data A/B and timing pending |
