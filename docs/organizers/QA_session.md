...
| our question | answer from the session |
|---|---|
| 1. staged obstacles in `new_data`? | no — only empty tunnel; no labels; the evaluation adds synthetic obstacles (facts 4, 5) |
| 2. control bag format like `new_data` / five of six bags? | control data: same conditions as the provided data, LiDAR mount of the empty-tunnel rides (fact 7); full-ride recordings included (fact 15). Topic / frame id not stated — the node keeps auto-discovery |
...
| 6. how is the result evaluated, which messages? | per-frame obstacle yes/no + distance, any ROS topic, extras welcome if described (fact 9) |
