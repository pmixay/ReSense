# Captain Log, 16–24.09

> **Purpose:** the captain's status log, findings and backlog of 16–24.09, kept as history
> (spec §8.7 "как менялось качество"); the current board is [`../CAPTAIN.md`](../CAPTAIN.md).
> **Audience:** team, jury (history) · **Owner:** P1 · **Language:** EN
> **Last verified:** not maintained; every number is that of its date · **Status:** archive

Moved on 24.09 from `docs/CAPTAIN.md` at `537e220` (its §0 "Status", lines 12–299, and §3–§4
"work streams" and "what the captain can do now", lines 334–431). The text is verbatim except:
...
5. **v0.5 on real data (21–22.09).** P3's real-data round (the captain committed its draft,
   measured every number and had the code reviewed independently): false alarms on the five obstacle-free bags 1 001 frames / 192
...
17. **Re-measurement on the current code (24.09)** (EXPERIMENTS "Re-measurement", raw
    `../evidence/results/experiments_2026-09-24_remeasure.json`): the organizers' data downloaded again and cached
...
18. **Independent review of 24.09** (commit `d58567e`; its scorecard and full report are in the git
    history, superseded by the criteria judgement in [`SCORECARD.md`](../SCORECARD.md)) replaced all
    earlier evaluations. It reproduced every real-data headline number and measured the opt-in near-bed
...
| P4 | done: extended-dataset intake; the injector's bed placement, set F axis anchoring and evaluation accounting corrected; paired set S on 108 original empty-bag frames; the full-rate six-bag R/E check; the paired set F on curve/edge scenes (nothing beyond 100 m) and on straight track (a person first confirmed at 154 m from the near-rail anchor, review item 7 of 24.09). **The organizers' 1 510-frame synthetic-obstacle bag is now labelled exactly** (`labels/cloud_with_fake_obj.json`) and graded per object, and the config sweeps and the signature experiment have been measured against the six original bags. Left: re-run the 20-minute ride for the P3 signature change; keep set O in every regression. Real reflectivity evidence is limited to the original person and rail object | `P4_AUDIT.md`, `DATASET.md` "Synthetic-obstacle recording", `EVALUATION.md` set O |
| P1 | send questions 1–2 of `QUESTIONS.md` (the return-mode and time questions were answered on 24.09, the talk-length question withdrawn): is the envelope in the organizers' check taken from the LiDAR's axis or from the rails (0.24° apart in their bag), and is "2х2 сверху габарита" an obstacle | four of the ten test objects change sides of the envelope edge between the two frames |
