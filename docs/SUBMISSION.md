...
| 14 | tests (spec §8.5) | `tests/` (130 tests: algorithm on the ray-cast tunnel, envelope and low objects, mount calibration and guards, the node's decision / fault / watchdog logic against ROS stand-ins in `tests/test_node.py`), `web/demo/`, CI (`pytest`, `web` and Docker jobs; the Docker job also plays a synthetic bag through the node) | P4 / P1 / P2 | done |
