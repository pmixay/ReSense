...
| 14 | tests (spec §8.5) | `tests/` (235 tests: algorithm on the ray-cast tunnel, envelope and low objects, mount calibration and guards, P4 placement, evaluation and the synthetic-obstacle labels, the node's decision / fault / watchdog / input-switching logic against ROS stand-ins in `tests/test_node.py`), `web/demo/`, CI (`pytest`, `lint` (ruff), `web` and Docker jobs; the Docker job also plays synthetic bags through the node, the second time the organizers' way: node and a uid-1000 player in separate containers) | P4 / P1 / P2 | done |
...
that burst one frame every 0.3 s of recording, so frames in between are skipped). `--expect-clear` on an empty bag is the
...
same with containers (node, player as uid 1000, status recorder). Expect `NO_INPUT` while the
player preloads the recording (2.6–4 s for 1.9 GB in the page cache, longer from a slow disk),
then the first seconds worked through within ~2 s; the first STOP on `doubleT_obstacle` comes
1.3–1.6 s into the recording (v0.6.4, EXPERIMENTS.md §3b; v0.6.3: 3.2–4.5 s).
