...
(7–10 fps, p95 112–130 ms), so the latency and drop criteria are checked on the team's 8-core
machine. Two transport bugs found and fixed: a best-effort input lost 196 of 201 clouds
(`input_reliability: auto`), and a normal user's player could not reach the root node through
shared memory (DDS over UDP). On the dry-run machine, play the bag from a normal user's console as
the organizers will on their stand (`scripts/console_test.sh` does the same with containers).
