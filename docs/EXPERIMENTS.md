...
**Reproduced with the ride on a second machine (26.09 evening).** In a fresh container (4 vCPU,
15 GB, no Docker) every cache was rebuilt from the organizers' links (set O from
`cloud_with_fake_obj.zst`, the six recordings from `Датасет.zip`, the ride from `new_data.zst`
split by split as in VM_GUIDE §2.3: 11 271 frames, 221 stamp files) and the strict gate ran with no
`--allow` against `_ride_p3d`: **PASS, every gated row the same** (the 3 ride and 15 set F straight
rows included, [`regression_gate_2026-09-26_head_fresh_machine.json`](evidence/results/regression_gate_2026-09-26_head_fresh_machine.json),
319 s at `--jobs 3`). The ride's 45 events / 3.5 per km and set F straight are therefore a second
machine's measurement, not only the archived baseline; the rate / re-mount checks (5 Hz 10 / 10,
+3° roll 14 / 16, +3° pitch 17 / 18), the 221-start census (18 starts with a STOP, 120 / 35 / 28)
and the ride-dependent test (`tests/test_rail_start.py`, 28 passed) reproduce too (§1p). The raw
set O bag and its 5 mm int16 cache are still not score-identical (P4, 26.09); the committed gate
stays cache-based.
...
| `tests/test_rail_start.py` with the ride cache (the test deselected everywhere else) | 28 passed, the ride-dependent one included | — |
