...
(the shipped defaults of `fa18832`: the P3 items of 26.09 with the STOP keep and its 10 s cap, §1o; six recordings,
set O, the ride and set F straight). It is the single check of these results: set O also gates
per-object sustained STOP distance, longest missed interval, and visible / STOP counts in every
distance bin (P4, 26.09). A change that moves them commits a new baseline, one that does not leaves
this table as it is. The one before it,
...
`gauge.axis_union` off; §1n), and the five before that,
...
**The P3 / P4 completion pass, 26.09 evening (another fresh machine).** In a second fresh
container (4 vCPU, 15 GB, no Docker) every cache was rebuilt again from the organizers' links (set O
1 510 frames, the six recordings 2 488 with their `metadata.yaml` equal to the committed originals,
the ride 11 271 frames from 221 split files) and the strict gate ran with no `--allow` against
`_ride_p3d`: **PASS, every gated row the same** ([`regression_gate_2026-09-26_head_fresh_machine.json`](evidence/results/regression_gate_2026-09-26_head_fresh_machine.json),
319 s at `--jobs 3`), so the ride and set F straight rows are now two machines' measurements. The
rate / re-mount checks, the 221-start census, the ride-dependent test, P4's candidate B and the
envelope union with its review fixes were run there too (§1p). P4's own strict command of the
afternoon had exited 1 only because the ride cache was absent on that workstation
([`p4_p3d_available_reference_2026-09-26.json`](evidence/results/p4_p3d_available_reference_2026-09-26.json)).
The raw set O bag and its 5 mm int16 cache are still not score-identical (387 / 7 / 1 against
384 / 6 / 3); the committed gate stays cache-based.
...
| P4 rate / mount stress and empty suffix | five bags false events / STOP episodes: 5 Hz **10 / 10**, +3° roll **14 / 16**, +3° pitch **17 / 18** since the safety review of 26.09 turned the refinement off (reproduced value for value on a second machine on 26.09 evening, the pitch's events listed in §1p) (with it, round 2 of 25.09: 10 / 10, 11 / 13, 16 / 17; P4's run of 25.09 before the rate / re-mount flags: 13 / 17 with 3 new events in `roundT_doubleT`, 16 / 17, 17 / 18); `roundT_doubleT` 0 / 2 / 1 events (0 / 1 / 0 with the refinement); `doubleT_obstacle` labelled hits 92 of 123 @ frame 12, 183 of 246 @ 12, 186 of 246 @ 11 (26.09; 181 of 246 at +3° roll before). Frames 804–1 509 of `cloud_with_fake_obj`: **1 false STOP frame / 1 event in 706 frames**, frame 1131 at 140.72 m; reproduced on the integrated P3 code from both the raw bag and quantized cache (§1g; the earlier zero-alarm report was stale) | real backgrounds, 25.09, 26.09 [measured] | §1g, §1i; [`scorecard13_2026-09-25.json`](evidence/results/scorecard13_2026-09-25.json), [`p3_round2_combined_2026-09-25.json`](evidence/results/p3_round2_combined_2026-09-25.json), [`p3_round2_review_fixes_2026-09-26.json`](evidence/results/p3_round2_review_fixes_2026-09-26.json) |
...
| offline timing per frame | **the shipped path (C++ kernels), 26.09:** 22.7–30.7 ms mean, p95 32.8–42.1 ms per frame on set O, `roundT_doubleT` (120°) and `doubleT_obstacle` (360°), one core, idle 4-core machine, `resense bench` (the re-judgement, [`evidence/rejudge_2026-09-26/bench_native.txt`](evidence/rejudge_2026-09-26/bench_native.txt)); the numpy path there 78.3 / 95.5 ms at 360°. Before: 42–64 ms mean, p95 53–78 ms on every recording, one core, numpy path (v0.6.3), health monitor not included (7–14 ms more); on another idle VM on 24.09 36.5–52.2 ms mean, p95 50.3–67.1 ms; the optional C++ kernels (merged 24.09, after v0.6.3) cut the detector time by 38–57 % with identical output | timing: sandbox, 23.09 (kernels: 24.09, under load) | §3; [`ARCHITECTURE.md`](ARCHITECTURE.md) "Native kernels" |
...
selected from this suffix. This was the P4 measurement on the pre-P3-round-2 code, not the current detector. Replaying the
same raw bag and its quantized cache on integrated P3 code gives one background STOP at frame 1131,
140.72 m, on both inputs ([`seto_suffix_p3c_2026-09-26.json`](evidence/results/seto_suffix_p3c_2026-09-26.json)).
The held-out segment remains a small sample and does not establish a per-hour false-alarm rate or
show that the 5 Hz / tilt cases are fixed.
...
### 1o. Range on the organizers' objects (26.09): what limits it; a STOP keep shipped

[organizers' synthetic and real, measured 26.09; P3.] The re-judgement of 26.09 scored 8.2 Range
at 7.5 / 15: one STOP frame beyond 100 m on set O, the 0.3 m cubes only from 43–53 m.

*Investigation* (`00143b0`, default config, every frame; wrappers that only record). In set O
the object points are the tail of every cached frame, so the returns per object and frame are
exact. Each object-frame was traced through the corridor, the strict envelope, the clustering,
the signatures, the tracker and the decision.

| object | returns a frame where it is missed | what rejects them | limit |
|---|---|---|---|
| `big_center` 2 × 2 m | 63–80 at 98.7 m (frame 0) | nothing: a gauge cluster from frame 0, a STOP after the 0.5 s confirmation (98.0 m) | data: the recording starts with it at 98.7 m |
| `big_above` 2 × 2 m, top | 20–56 at 136–220 m | by the extrapolated height reference its bottom is 3.1–4.3 m up, above the 3.0 m top (no corridor return in 45 of 49 frames) | height reference |
| | 36–63 at 117–134 m | one ring inside, 0.00–0.01 m tall: dropped by `min_height` (0.08 m) | `min_height` |
| `big_above` after its STOP at 101.3 m | 86–129, 18–66 inside | `elevated` from 87 m (2.0–2.4 m wide, bottom 2.1–2.6 m): the zone vote drops the STOP at 77 m; at 46–33 m one ring inside (`min_height`) four frames in a row kills the track; the new track is `elevated` | rules |
| `long_low_on_rails` plank | 2–8 at 97–131 m; 6–11 at 84–96 m | 97–131 m: 0.0–1.0 m under the rail head by the reference, below the envelope floor; 84–96 m: one ring (`min_height`) in 5 of 9 frames | height reference, `min_height` |
| `small_center` 0.3 m cube | 1–2 beyond 100 m, 2–4 at 60–100 m | 80–120 m joined to a line at the corridor edge (`max_extent`); 60–80 m 4 voxels < `min_points` 5 | physics |
| `small_on_rail` 0.3 m cube | 2–6 at 55–82 m | 3.0–4.3 m above the rail head at 70–82 m; 1.1–1.4 m off the axis at 55–69 m | placement, physics |
| `thin_hanging` 5 cm | 1–8 | above the top beyond 50 m; 1 return a frame at 40–43 m | physics |

**Physics limits four objects, the height reference two.** Beyond 100 m the 0.3 m cubes and the
hanging rod return 1–4 points a frame. `big_above` and the plank return enough points, but the
extrapolated rail level moves ±1 m at 150–220 m and puts them above the top or under the floor.

*What relaxing the limits would admit* (the six recordings and the ride, same wrappers):
* at 100–200 m within 0.9 m of the axis, ≥ 3 returns lie just above the top (3.0–3.6 m) in 45 %
  of the ride's frames (63 % of the five recordings') and just under the floor in 60 % (62 %);
* flat gauge slivers (≥ 3 strict voxels, under 0.08 m tall) in 696 ride frames and 399
  five-recording frames at 40–260 m;
* 3–4 voxel clusters with a strict voxel at 60–100 m in 278 ride cluster-frames.

A taller envelope, a lower floor, or flat or 3–4 voxel clusters as new tracks were therefore not
candidates. On the real data the shape signatures almost never hit a confirmed STOP (one
cluster-frame logged on the ride). On set O they take down the box at the envelope top.

*Pre-registered* at 09:15:16 UTC, commit `ec41e4c` (09:15:23 UTC), on `00143b0`, before any
candidate run ([`p3_range_2026-09-26.json`](evidence/results/p3_range_2026-09-26.json)). Two
tracker flags:
* `tracking.stop_keep_signature`: a track that was a STOP in the previous frame counts a cluster
  demoted only by a shape signature (`elevated`, `floating`, `edge`, `wall_face`; never a column,
  the far-field reasons or `overhead`) as inside the envelope. It never makes a new obstacle.
* `tracking.stop_keep_thin` 1 / 2: a scan line inside the envelope (flatter than `min_height`) may
  continue a track nothing else matched: 1 a STOP track, 2 also an unreported track whose last
  hit was inside. It never starts a track.
...
(frames 40–41). C also moved #8's first STOP to 109.5 m and the plank's to 83.8 m, the only
first-STOP gains of the round.

**Round 1: B not shipped.** Its gate passes: no gated row is worse, and #8 is a STOP on every
frame from 101.3 m. But the ride's alarm frames rise by 9 (events 45 and STOP episodes 38 the
same), and the ship rule does not allow that. The 9 frames extend two false STOPs of the default
detector:
* 6 frames at 113–115 m (`new_data_192`): a kept `floating` cluster of 6 voxels held the zone vote;
* 3 frames at 98–100 m (`new_data_208`): a bed scan line of 7 voxels (4.7 × 2.5 m, 0.02 m tall)
  continued the track.

The ride was compared frame by frame with the flags off on the same code.

**Round 2** (pre-registered at 10:12:18 UTC, commit `51dee80`, before its code): B10 = B, but a
kept cluster needs ≥ 10 voxels inside the strict envelope (`tracking.stop_keep_min_voxels`). That
is the bar the shipped near escalation uses; it is not tuned here. It was written after seeing
round 1's two ride tracks, so its ride result is in-sample on them. #8's kept clusters have 16–66
strict voxels.

**B10 passes and ships** (full gate against `_ride_p3c`, `ffb3fe7` with `--set`; the new defaults
`6c76605`: the same run frame by frame, 15 269 frames, config sha256 `7d93c419…`):
...
* #8 is now a STOP on every visible frame from 101.3 m to 6.8 m. 44 set O frames carry a
...
(`scripts/output_fingerprint.py --cache /data/cache --jobs 1`, 2 930 frames each; on the final
code with every `stop_keep` flag off again identical to `208152d`). Tests 555 → 586 after the
merges, the same with `RESENSE_NATIVE=0` (`tests/test_stop_keep.py`, +28: tracker, clustering,
ray-cast, the cap; +3 P4's gates); `web/demo` 13. In the ray-cast test a box at the envelope top reads `elevated`, then one ring. It
loses its STOP with the rules off, keeps it with them, and an `elevated` beam that was never a STOP
stays advisory.

**Safety review of B10 (26.09): non-blocking, clean on safety.** Its findings:
* The association is unchanged. The signature keep changes only the zone vote of a matched track
  that was a STOP in the previous frame. Scan lines only continue STOP tracks that nothing else
  matched (mode 1). Neither starts or confirms a track.
* Distance: frame 663's +1.55 m is a scan-line continuation within the 3 m match. A scan line
  inside the association gate can continue a track after its object is gone. That is an accuracy
  issue, not a safety one.
* The 10-voxel bar is in-sample (round 2 was written after round 1's two ride tracks).
* Exposure on 1 200 ride frames (the reviewer's sample): 0 qualifying scan lines, 1 `elevated`
  cluster.
* Nothing ended a kept STOP in time. At a standing train, a false STOP beyond 35 m kept by
  demoted or scan-line clusters with ≥ 10 strict voxels could last indefinitely.

*The cap, taken* (`tracking.stop_keep_max_s` 10 s; 0 = no cap). The keep rules act on a track only
while its last clean hit (an obstacle cluster inside the envelope, not a scan line) is at most 10 s
of sensor time ago. The time is summed from the measured frame intervals, so it is the same at 10
and 5 Hz. Past the cap the track falls back to the behaviour without the keep rules; a clean hit
resets it, and a track never seen clean is never kept. #8's longest keep-only stretch is 4.3 s by
the receive stamps (frames 641–683 after its last clean hit at 85.2 m); its whole STOP run
(frames 632–683) is 5.1 s. +6 tests: a kept STOP at a standing train ends after the cap at 10 and
5 Hz, a scan-line continuation too, a 5.1 s moving approach is unchanged, a clean hit resets.
*The final gate, with the cap* (`fa18832`: the cap, and the working branch `dcf7f90` merged with P4's
new set O gates of held-from, missed intervals and per-bin counts; `--jobs 1` against the augmented
`_ride_p3c`, 791 s): PASS, 4 gated rows better, none worse. #8: STOP frames 22 → 51, held-from
(scorer) 23.9 → 111.4 m, 0–50 m bin 10 → 25 and 50–100 m bin 11 → 25 STOP frames. The missed
interval of frames 644–672 (77.2 → 23.9 m, 29 frames) is gone; the one before the first STOP
(frames 559–631, 248 → 103 m) stays. The ride stays 183 / 45 / 38. Every frame of the six
recordings, set O and the ride equals the B10 run of `ffb3fe7` (15 269 frames: decision,
detections, zones, reasons, distances): **the cap changes nothing on the recorded data.**

*New gate baseline:*
[`regression_baseline_2026-09-26_ride_p3d.json`](evidence/results/regression_baseline_2026-09-26_ride_p3d.json),
the final gate's run (`fa18832`, with the cap and P4's new set O metrics) without its gate block.
`LC_ALL=C sort` picks it (VM_GUIDE §4.4). The same run against it (`--from-json`): PASS, every row
the same. Gate outputs: [round 1, B](evidence/results/regression_gate_2026-09-26_range.json),
[round 2, B10](evidence/results/regression_gate_2026-09-26_range_b10.json),
[final, with the cap](evidence/results/regression_gate_2026-09-26_range_final.json).

### 1p. Evening of 26.09: the baseline reproduced with the ride on a second machine; P4's candidate B completed; the union with the review fixes; +3° pitch; the census on the head

[real, organizers' synthetic and synthetic, measured 26.09 evening; P3 / P4 by an agent, on the
captain's order to complete the P3 and P4 work.] Pre-registered before any run in
[`p3_p4_prereg_2026-09-26_evening.json`](evidence/results/p3_p4_prereg_2026-09-26_evening.json)
(commit `53b75d4`, 15:32:50 UTC). Machine: a fresh cloud container, 4 vCPU, 15 GB, no Docker
daemon; the caches rebuilt from the organizers' links (set O 1 510 frames, the six recordings
2 488 with their `metadata.yaml` equal to the committed originals, the ride 11 271 frames from
221 split files). Detector code: `fa18832`'s (only tests and docs changed since).

| run | result | JSON |
|---|---|---|
| set O alone (`resense run --npy` + `score_fake_objects.py`) | every object equal to `_ride_p3d` (384 / 801, 6 outside, 3 background frames / 2 ids, 415 alarm frames) | — |
| the gate on the six recordings + set O (`--allow` the ride and set F, before the ride was cached) | PASS, every row the same, 74 s at `--jobs 2` | `/data/out/gate_avail` (not committed: superseded by the full run) |
| **the strict gate with the ride and set F straight, no `--allow`** | **PASS, every gated row the same**: five bags 58 / 13 / 16, ride 183 / 45 / 38, set O every object, set F straight all 15 rows (person 151.0 m, trolley 151.4 m, crate 123.9 m, cable 98.9 m, 0.5 m box 1 of 6 at 51.9 m); 319 s at `--jobs 3` | [`regression_gate_2026-09-26_head_fresh_machine.json`](evidence/results/regression_gate_2026-09-26_head_fresh_machine.json) |
| P4's candidate B, `tracking.near_escalate_voxels` 10 → 8, the full gate | PASS, 6 gated rows better, none worse: set O #4 (edge cube) 2 → 3 STOP frames, first STOP 5.2 → 7.1 m, held from 7.1 m, inside STOP frames 384 → 385; the ride 183 / 45 / 38, the five bags, `doubleT_obstacle`, set F straight, #5 / #7 / background identical | [`regression_gate_2026-09-26_p4_candidate_B_full.json`](evidence/results/regression_gate_2026-09-26_p4_candidate_B_full.json) |
| candidate B's acceptance checks (P4's frozen rules): census, 5 Hz / roll / pitch, start offsets | all pass with numbers identical to the head: census 18 / 120 / 35 / 28 on the 221 ride splits (none within 10 m), the eight pieces 1 / 18 / 1 / 1, the seven recordings 3 / 68 / 6 / 3; five bags as recorded 13 / 16, 5 Hz 10 / 10, +3° roll 14 / 16, +3° pitch 17 / 18; start offsets 0 / 10 / 20 / 30 / 40 → 13 / 16, 13 / 14, 13 / 12, 10 / 11, 7 / 11 (P4's P3d reference value for value); `doubleT_obstacle` unchanged at every offset. B changes nothing but set O #4 ([`p4_candidate_B_checks_2026-09-26/`](evidence/results/p4_candidate_B_checks_2026-09-26/)) | [`p4_completion_2026-09-26_evening.json`](evidence/results/p4_completion_2026-09-26_evening.json) |
| the union `gauge.axis_union` 1 with the review fixes of §1m, the full gate (never measured before) | PASS, 6 gated rows better, none worse: set O #4 2 → 9 STOP frames, first STOP 5.2 → 18.2 m, held from 20.1 m, inside STOP frames 384 → 391; #6, #8, #5 / #7, background, the ride (183 / 45 / 38), the five bags, `doubleT_obstacle` and set F straight identical | [`regression_gate_2026-09-26_axis_union_fixed_full.json`](evidence/results/regression_gate_2026-09-26_axis_union_fixed_full.json) |
| rate / re-mount on the six recordings (`robustness_check.py`) | as recorded 13 / 16, 5 Hz **10 / 10**, +3° roll **14 / 16**, +3° pitch **17 / 18** events / STOP episodes: P4's numbers of the P3d run, value for value | [`p4_robustness_2026-09-26_evening/`](evidence/results/p4_robustness_2026-09-26_evening/) |
| the start-up census on the head (221 ride splits, 8 pieces, 7 recordings; 40 frames each) | ride splits: 18 of 221 starts with a STOP, 120 STOP frames / 35 events / 28 episodes, 8 events in the first 1.5 s, **0 within 10 m** (the 2.9 m finding of §1j is gone: the ride pieces give 1 start / 18 frames / 1 event, the 138 m one, against 2 / 22 / 2 on `867bb8a`); every event under calibration `pending`; the seven recordings 3 starts / 68 / 6 / 3 as in P4's run | [`p4_census_p3d_2026-09-26.json`](evidence/results/p4_census_p3d_2026-09-26.json) |
| `tests/test_rail_start.py` with the ride cache (the test deselected everywhere else) | 28 passed, the ride-dependent one included; the whole suite 586 at the time (587 since the video test added the same evening) | — |

**What the +3° pitch adds** (five bags 13 → 17 events, 16 → 18 STOP episodes; the same events as
...
* `roundT_doubleT` +1: a `low` cluster 0.3 m across at 48 m in frames 99–100 with its bottom 0.13 m
...
* **Candidate B is eligible by P4's frozen rules** (the full gate passes with no missing row and
  the ride not up; its checks below) but is **not shipped by this session**: the gain is one STOP
  frame at 7 m on an object that straddles the envelope edge (§1m: inside from the rails in 16 of
  83 frames), the change lowers the near escalation's voxel bar for every demoted track, and the
  detector freeze of 26.09 20:00 (CAPTAIN action 18) is the captain's. A default change would
  re-cut the baseline (`--from-json` of the B gate run) and needs no deck rebuild (no headline
  number moves).
* **The union stays off.** With the review fixes it passes the full gate for the first time and
  wins #4 seven frames (first STOP 18.2 m), nothing else moves; but the organizers' Q1 (which axis
  the hidden check uses) is unanswered and the re-review's open item (a union part that passes the
  oversize split but took in an edge-line stretch is judged by the shape rules on that part)
  stands. Recorded for the captain's decision when Q1 is answered.
* **Nothing else shipped**; the baseline `_ride_p3d` stands, now reproduced on a second machine.
...
first run. **The drop criterion changed between the two runs** (disclosed by the re-judgement of
26.09): the first run FAILED on "20 dropped after 5 s"; `5be4143` then counted drops after the later
of 5 s and the end of the start-up catch-up, and against the recording's own messages, because 16
of the 20 were the catch-up's deliberate skips and 4 are missing from the recording. The second
run's PASS is against the redefined criterion; its drops (51, 43 of them the catch-up's) would
have failed the first one. The re-judgement's own warm run on a third machine also PASSES the
redefined criterion, and FAILS it from a cold disk (README "Acceptance test and CI").
...
- **5 Hz and a ±3° re-mount** (SCORECARD #13; P3; §1g, §1i): the causes fixed on 25.09 (five
  ...
  0 / 1 / 0; the same on the combined round-2 defaults); on the P3d head 5 Hz 10 / 10, roll 14 / 16,
  pitch 17 / 18 (26.09 evening, §1p, reproduced on a second machine): the pitch's four added events
  are 1–4 frame flickers, three of them under `provisional` calibration (a low cluster at 1–3 m, a
  low cluster 0.13 m below the tilted rail head at 48 m, a 1.5 m cluster at 112 m in one frame) and
  one an edge structure at 87 m; the switch and platform ids only re-split. No bounded candidate
  (a rule on `provisional` alone fails §1j's S1); open;
...
- **the envelope union** (`gauge.axis_union` 1; §1m): with the review fixes it passes the full gate
  with the ride (26.09 evening, §1p: set O #4 2 → 9 STOP frames, first STOP 18.2 m, nothing else
  moves); off until the organizers answer Q1 and the re-review's open item is closed (the shape
  rules on the rails-only part of a union cluster);
