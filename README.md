...
python scripts/regression_gate.py --cache /data/cache \
    --baseline docs/evidence/results/regression_baseline_2026-09-26_ride_startup.json   # the gate for every detector change (exit 1 = worse, or a baseline set missing here)
```
...
| `tracking.low_min_seen_distance` | 4 m | since 26.09 (start-up of a fresh bag): a low (bed-level) track is reported only once it has been matched this far or farther, where the bed cross-section is learned; a fresh start at a standing train STOPped at 2.9–3.1 m on the rail heads; ride 46 → 45 events, everything else identical (EXPERIMENTS §1j). The start-up candidates `lowobj.pending_advisory` and `lowobj.min_model_age` stay off: they delay a real low object at a fresh start |
