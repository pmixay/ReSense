...
- **Start-up of a fresh bag (26.09, P3 / P4): `tracking.low_min_seen_distance` 4 m, on.** A
  ...
  events). The warm detector STOPs more on the same frames, so this is not a start-up effect.
  Pre-registered a / b / c: a (low advisory while the calibration is pending) and b (a minimum
  model age) fail the ray-cast check (a real low object 20 / 40 m ahead is not STOPped / STOPped
  12 frames late). c, a low track reported only once matched at ≥ 4 m, passes: census 41 → 40
  false events, gate PASS, ride 187 / 46 / 39 → 183 / 45 / 38, everything else identical frame by
