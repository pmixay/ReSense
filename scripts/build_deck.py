...
def s_results(sl):          # template slide 20: left card + five rows
    ...
    rows = [
        ...
        f"{N['fake_plank']} м, кубы 0,3 м — с {N['fake_cubes']} м; висящий 5 см — с 30 м, ящик у края пропущен",
...
def s_hard(sl):             # template slide 15: five lined rows
    ...
    rows = [
        ...
        "лидара (−0,24° к рельсам), наш габарит — от рельсов; висящий предмет 5 см — STOP только с 30 м",
    ]
    for idx, text in zip(range(15, 20), rows):
...
NOTES = {  # speaker notes per template slide (the main shot's are set in s_hero)
    ...
    20: "Человек на пути — 58 из 61 кадра, предмет на рельсе — 125 из 126. Ложных событий в 20-минутной "
        ...
        "ящик — с 98 м, кубы 30 см — только с 43–53 м; два объекта без STOP, о них — через слайд.",
    ...
    21: "Это единственная внешняя проверка — объекты, которые вставили сами организаторы, поезд едет к ним до "
        ...
        "только с 43–53 м: на 60–115 м от них 2–4 точки в кадре. Висящий предмет 5 см — с 30 м; ящик у края "
        "габарита пропущен: объекты ставили от оси лидара, а наш габарит идёт по рельсам.",
