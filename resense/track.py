...
def estimate_axis_from_walls(xyz: np.ndarray, model: TrackModel, cfg: TrackConfig,
    ...
    boundary wins. Returns (tan_yaw, curvature, quality, x_valid, n_sides, disagreement, x_valid_min)
    or None; ``disagreement`` is the curvature difference of the two sides when both were fitted;
    ``x_valid`` / ``x_valid_min`` are the last observed bin of the longer / shorter side kept.
    """
    X, Y, Z = xyz[:, 0], xyz[:, 1], xyz[:, 2]
    ...
    return c1, c2, quality, x_valid, n_sides, disagreement, x_valid_min
...
def estimate_track(xyz: np.ndarray, cfg: TrackConfig, prev: Optional[TrackModel] = None) -> TrackModel:
    ...
    if cfg.walls_enabled:
        ...
        if est is not None:
            tan_yaw, curv, q, x_valid, n_sides, disagreement, x_valid_min = est
