...
class ClusterConfig:
    ...
    # 25.09 (SCORECARD #11), opt-in, off: thin objects hanging from above into the envelope near the
    # axis (clustering.find_hanging): the organizers' 5 cm object dips 0.2-0.4 m below the envelope
    # top with 1-3 returns a frame and never reaches min_points; linked to its part above the top it does
    hanging_enabled: bool = False
    ...
    hanging_min_voxels: int = 2        # voxels inside the strict envelope (at least one more above its top)
