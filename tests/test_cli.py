...
def test_eval_npy_labelled_only_and_stride(labelled_npy):
    ...
    out = main(["eval", "--npy", str(d), "--gt", str(d / "gt.json"), "--every", "2", "--repeat", "3"])
...
def test_eval_gt_bbox_row(labelled_npy, tmp_path):
    ...
    out = main(["eval", "--npy", str(d), "--gt", str(p), "--repeat", "3"])
...
def test_catalogue_spec_and_unknown_name():
    ...
    with pytest.raises(SystemExit):
        main(["inject", "--npy", "/nonexistent", "--out", "/tmp/never", "--kinds", "piano"])
...
def test_inject_static_roundtrip(synth_npy_dir, tmp_path):
    ...
    out = tmp_path / "inj"
    main(["inject", "--npy", str(synth_npy_dir), "--out", str(out), "--kinds", "box0.5,person",
...
def test_inject_sequence_and_augment_roundtrip(synth_npy_dir, tmp_path):
    ...
    out = tmp_path / "seq"
    main(["inject", "--npy", str(synth_npy_dir), "--limit", "1", "--out", str(out), "--kinds", "box1.0",
    ...
    out2 = tmp_path / "neg"
    main(["inject", "--npy", str(synth_npy_dir), "--limit", "1", "--out", str(out2), "--kinds", "plank",
